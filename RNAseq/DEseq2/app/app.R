  ...
  tabsetPanel(
    ...
    tabPanel('数据格式',
             fluidRow( 
               ...
               column(width = 7, 
                      box(
                        title = "矩阵：(excel中，表1为矩阵，基因列名需为：ID)",
                        width = NULL, 
                        height = 10, 
                        status = "primary",
                        solidHeader = T) 
               ...
               column(width = 3 , 
                      box(
                        title = "分组：(表2列名：group 与 condition)",
                        width = NULL, 
                        height = 10, 
                        status = "primary",
                        solidHeader = T) )
    ...
    tabPanel('基础图形',
             ...
             fluidRow( 
               ...
               column(width = 2,
                      downloadButton("downloadPlots", "下载图形：.RDS文件") )
               ),
             tags$br(),
             fluidRow( 
               column(width = 12,
...
server <- function(input, output, session) {
  ...
  myDEseq1 <- reactive({
    ...
    file1 <- input$file
    if(is.null(file1)){
      df1 <- data.frame(read_excel('/srv/shiny-server/RNAseq/file/RNAseq.xlsx',1) )
      df1 <- head(df1,2000)
      
      df2 <- data.frame(read_excel('/srv/shiny-server/RNAseq/file/RNAseq.xlsx',2) )
    } 
    else{
...
observe({
    ...
    if(is.null(file1)){
      # 显示参考数据，分组
      df2_deseq <- data.frame(read_excel('/srv/shiny-server/RNAseq/file/RNAseq.xlsx',2) )
    } 
    else{ 
      # 显示输入数据
...
output$sample2_group <- renderDataTable({
  ...
  file1 <- input$file
  if(is.null(file1)){
    return(data.frame(read_excel('/srv/shiny-server/RNAseq/file/RNAseq.xlsx',2) ))
  }
  ...
  output$downloadSampleData <- downloadHandler(
    filename = function() {
      paste('DEseq2 参考数据.xlsx')
    },
    content = function(file) {
      
      sample1 <- head( as.data.frame( read_excel('/srv/shiny-server/RNAseq/file/RNAseq.xlsx',1) ), input$sample_exp_row)
      sample2 <- as.data.frame( read_excel('/srv/shiny-server/RNAseq/file/RNAseq.xlsx',2) )
